...
/**
 * @table Import.ArticoloCategoriaMaster
 * @description Mappatura Articolo / Categoria Master
*/

--DROP TABLE IF EXISTS Import.ArticoloCategoriaMaster;
GO

IF OBJECT_ID('Import.ArticoloCategoriaMaster', 'U') IS NULL
BEGIN
    ...
    INTO Import.ArticoloCategoriaMaster
    ...
    ALTER TABLE Import.ArticoloCategoriaMaster ADD CONSTRAINT PK_Import_ArticoloCategoriaMaster PRIMARY KEY CLUSTERED (id_articolo, CategoriaMaster);

    ALTER TABLE Import.ArticoloCategoriaMaster ALTER COLUMN CategoriaMaster NVARCHAR(40) NOT NULL;
    ALTER TABLE Import.ArticoloCategoriaMaster ALTER COLUMN CodiceEsercizioMaster NVARCHAR(10) NOT NULL;
    ALTER TABLE Import.ArticoloCategoriaMaster ALTER COLUMN Percentuale DECIMAL(5,2) NOT NULL;

    CREATE NONCLUSTERED INDEX IX_Import_ArticoloCategoriaMaster_Codice ON Import.ArticoloCategoriaMaster (Codice);

    DELETE FROM Import.ArticoloCategoriaMaster WHERE Codice IN (N'MS2010', N'MS2011');

    INSERT INTO Import.ArticoloCategoriaMaster (
        id_articolo,
        Codice,
        Descrizione,
        CategoriaMaster,
    ...
    UPDATE Import.ArticoloCategoriaMaster SET CodiceEsercizioMaster = N'2022/2023' WHERE Codice = N'FO2286';

    -- Richiesta del 23/5/2025
    WITH ArticoliMaster2526
    AS (
        SELECT N'MS2409' AS Codice,
            N'Master Fisco 25/26 con crediti' AS Descrizione,
            N'Master MySolution' AS CategoriaMaster

        UNION ALL SELECT N'MS2410', N'Master Fisco 25/26 senza crediti', N'Master MySolution'
        UNION ALL SELECT N'MS2411', N'Master Fisco plus 25/26', N'Master MySolution'
        UNION ALL SELECT N'FO2564', N'Mini Master Revisione Legale Luglio Con crediti', N'Mini Master Revisione'
        UNION ALL SELECT N'FO2565', N'Mini Master Revisione Legale Luglio Senza crediti', N'Mini Master Revisione'
        UNION ALL SELECT N'FO2566', N'Mini Master Revisione Legale Ottobre Con crediti', N'Mini Master Revisione'
        UNION ALL SELECT N'FO2567', N'Mini Master Revisione Legale Ottobre Senza crediti', N'Mini Master Revisione'
        UNION ALL SELECT N'MS2412', N'Master Lavoro 25/26 con crediti', N'Master MySolution'
        UNION ALL SELECT N'MS2414', N'Master Lavoro 25/26 senza crediti', N'Master MySolution'
        UNION ALL SELECT N'MS2413', N'Master Lavoro plus', N'Master MySolution'
    )
    INSERT INTO Import.ArticoloCategoriaMaster (
        id_articolo,
        Codice,
    ...
    SELECT
        COALESCE(A.id_articolo, -1000-ROW_NUMBER() OVER (ORDER BY AM2526.Codice)) AS id_articolo,
        AM2526.Codice,
        AM2526.Descrizione,
        AM2526.CategoriaMaster,
        N'2025/2026',
        1.0

    FROM ArticoliMaster2526 AM2526
    LEFT JOIN Dim.Articolo A ON A.Codice = AM2526.Codice
    LEFT JOIN Import.ArticoloCategoriaMaster ACM ON ACM.Codice = AM2526.Codice
    WHERE ACM.id_articolo IS NULL;
