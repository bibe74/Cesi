...
BEGIN
    ...
    AS (
        ...
        INNER JOIN Dim.Data DFC ON DFC.PKData = CA.PKDataFineContratto
        WHERE CA.IsDeleted = CAST(0 AS BIT)
            AND CA.HasRoleMySolutionDemo = CAST(1 AS BIT)
            AND (
                @TipoCliente IS NULL
                OR CA.TipoCliente = @TipoCliente
            )
...
EXEC Fact.usp_ReportFatturatoGenerale @AnnoCorrente = @AnnoCorrente,
                                     @CapoArea = @CapoArea;
GO
...
CREATE OR ALTER PROCEDURE Fact.usp_ReportCruscottoClienti (
...
CREATE OR ALTER PROCEDURE Fact.usp_ReportDomandeMIA (
    @StartDate DATE NULL,
    @EndDate DATE NULL
)
AS
...
EXEC Fact.usp_ReportDomandeMIA
    @StartDate = @StartDate,
    @EndDate = @EndDate;
GO
...
CREATE OR ALTER PROCEDURE Fact.usp_ReportCreditiMIA (
    @CapoArea NVARCHAR(60) = NULL
)
AS
BEGIN
